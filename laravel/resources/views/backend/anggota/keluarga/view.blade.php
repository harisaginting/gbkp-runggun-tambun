                            ...
                            <div class="row">
                                <div class="col-md-12 col-sm-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" id="nama_keluarga" name="nama_keluarga" type="text" placeholder="Nama Keluarga" required readonly>
                                    </div>
...
var Page = function () {
    getKeluarga = async() =>{
          ...
          console.log(base_url+"/api/v1/keluarga/get/"+uuid);
          return await fetch(base_url+"/api/v1/keluarga/get/"+uuid, {
          ...
          init: function() { 
             $(document).on("click","#btn-save", function(e){
                  ...
                  if($("#form-keluarga").valid()){
                      ...
                      saveAnggota().then((data)=>{
                        ...
                        if(data.status == 200){
                            alert("data keluarga berhasil dirubah");
                            location.reload();
                        }else{
             ...
             global.on("click","#btn-add-anggota-save",()=>{
               if($("#form-anggota").valid()){
                  ...
                  $(btn_delete_anggota).addClass("btn btn-disable btn-sm btn-delete-anggota pull-right");
             ...
             $("#status").select2({
              ...
              getKeluarga().then((data)=>{
                $("#loader").addClass("hidden");
                if(data.status == 200){
                    let keluarga        = data.data.keluarga;
                    let keluarga_detail = data.data.keluarga_detail;
                    console.log(keluarga);
                    console.log(keluarga_detail);

                    $("#id_keluarga").val(keluarga.id);
                    $("#nama_keluarga").val(keluarga.nama);
                               
                    keluarga_detail.map((value,index) => {
                          let id_anggota    = value.id_anggota;
                          let nama_anggota  = value.nama_depan + " " +value.nama_belakang;


                          let id_status   = value.status_anggota;
                          let nama_status = value.status_label;

                          let anggota = {
                              anggota :  value.id_anggota,
                              status  :  value.status_anggota
                          }; 

                          list_anggota.push(anggota);

                          let container_anggota   = document.createElement("tr");
                          $(container_anggota).attr("id", "row-" + id_anggota);
